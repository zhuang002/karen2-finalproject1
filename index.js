...
function renderQuote(root) {
    root.innerHTML="<div style='background-color:yellow;padding:20;text-align:center;vertical-align:middle'>"
        +"<b style='font-size:20; font-family:sans-serif'>WELCOME TO EMC MILLWORK & CONSTRUCTION LTD</b>"
        +"</div>";
    
    let wizard = new Wizard(onWizardFinish);
    root.appendChild(wizard.getRoot());
}
...
class Gallery {
    ...
    setData(infos) {
        for (var i=0;i<infos.length;i++) {
            var info = infos[i];
            var textPicture = new TextPicture(info,320,400);
